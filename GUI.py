...
boardframe.grid(column=0,row=1,sticky=(N, W) )
...
for row in range(5):
    for col in range(5):
        #e[row][col] = Entry(boardframe,width=2,textvariable=letters[row][col], validate='key', validatecommand=vcmd,invalidcommand=inv,background='white')
        ...
        e[row][col].bind("<Button-3>", chgcolor)
